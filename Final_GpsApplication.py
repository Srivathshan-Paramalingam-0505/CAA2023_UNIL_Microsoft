...
statusLabel = M5Label('Fetching the informations', x=0, y=77, color=0x000, font=FONT_MONT_14,parent=None)
...
if response.status_code == 200:
    ...
    for i in range(len(latitude_list)-1):
      current_latitude = latitude_list[0] #used to test
      current_longitude = longitude_list[0] #used to test
      
      #As long as the biker does not reach coordinates that correspond to the next step
      while (current_latitude != latitude_list[i]) or (current_longitude != longitude_list[i]):
